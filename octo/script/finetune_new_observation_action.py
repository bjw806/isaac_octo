...
batch_size = 4
data_dir = "mnt/dataset/0.0.6/"
save_dir = "/home/sites/octo/mnt/train/"

train_iterator = 10000
...
def main(_):
    ...
    wandb.init(name="finetune_agv", project="octo")
    ...
    # dataset = tfds.builder_from_directory(data_dir).as_dataset(split='all')
    dataset = make_single_dataset(
        dataset_kwargs=dict(
            name="agv_example",
            data_dir=data_dir,
            image_obs_keys={"primary": "rcam_rgb"},
            depth_obs_keys={"primary": "rcam_depth"},
            proprio_obs_key="joint_pos",
    ...
    def process_batch(batch):
        ...
        batch["task"]["language_instruction"] = text_processor.encode(
            [
                "First, move the pin to the hole's XY coordinate only in the XY plane. Then, raise the Z-axis and plug the pin into the hole."
            ]
            * batch_size
        )
    ...
    for i in tqdm.tqdm(range(train_iterator), total=train_iterator, dynamic_ncols=True):
