...
class Se3KeyboardAGV(DeviceBase):
    ...
    * gripper: a binary command to open or close the gripper.
    ...
    def advance(self) -> tuple[np.ndarray, bool]:
        ...
        # zeros = np.zeros(3)
        # return np.concatenate([self._delta_pos, zeros])
    ...
    def _create_key_bindings(self):
        ...
        self._INPUT_KEY_MAPPING = {
            # x-axis (forward)
            "S": np.asarray([.3, 0.0, 0.0]) * self.pos_sensitivity,
            "W": np.asarray([-.3, 0.0, 0.0]) * self.pos_sensitivity,
            # y-axis (left-right)
            "A": np.asarray([0.0, .3, 0.0]) * self.pos_sensitivity,
            "D": np.asarray([0.0, -.3, 0.0]) * self.pos_sensitivity,
            # z-axis (up-down)
