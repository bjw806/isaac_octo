...
class PPO(Agent):
    ...
    def init(self, trainer_cfg: Optional[Mapping[str, Any]] = None) -> None:
        ...
        if self.memory is not None:
            ...
            self.memory.create_tensor(name="states", size=compute_space_size(self.observation_space), dtype=torch.float32)
    ...
    def record_transition(self,
        ...
        if self.memory is not None:
            ...
            image_flat = states["image"].reshape(states["image"].size(0), -1)         # Shape: [3 * H * W]
            image_flat2 = next_states["image"].reshape(states["image"].size(0), -1)         # Shape: [3 * H * W]
            combined_tensor = torch.cat((states["value"], image_flat, states["critic"]), dim=1)
            combined_tensor2 = torch.cat((next_states["value"], image_flat2, next_states["critic"]), dim=1)
            ...
            self.memory.add_samples(states=combined_tensor, actions=actions, rewards=rewards, next_states=combined_tensor2,
                                    terminated=terminated, truncated=truncated, log_prob=self._current_log_prob, values=values)
            for memory in self.secondary_memories:
                memory.add_samples(states=combined_tensor, actions=actions, rewards=rewards, next_states=combined_tensor2,
                                   terminated=terminated, truncated=truncated, log_prob=self._current_log_prob, values=values)
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        with torch.no_grad():
            ...
            state = {
                'value': self._state_preprocessor(self._current_next_states["value"].float()),
                'image': self._state_preprocessor(self._current_next_states["image"].float()),
                'critic': self._state_preprocessor(self._current_next_states["critic"].float()),
            }

            last_values, _, _ = self.value.act({"states": state}, role="value")
        ...
        for epoch in range(self._learning_epochs):
            ...
            for sampled_states, sampled_actions, sampled_log_prob, sampled_values, sampled_returns, sampled_advantages in sampled_batches:
                ...
                critic_flat = sampled_states[:, value_flat_size + image_flat_size:]
                
                channel_size = sampled_states.shape[0]
